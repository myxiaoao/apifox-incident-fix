...
- **恶意指标：**
  ...
  - **可疑字段（数据窃取字段，也可能为正常 Apifox 数据）：** HTTP 请求头中包含 `af_uuid`、`af_os`、`af_user`、`af_name`、`af_apifox_user`、`af_apifox_name`；读取 `common.accessToken` 凭证；执行 `ps aux` / `tasklist` 命令
- **修复版本：** 2.8.19+
...
| 模块 | 功能 |
|------|------|
| 0 - 取证确认 | 两层 LevelDB 检测：确认的恶意标记（`_rl_mc`/`_rl_headers`）触发红色告警，可疑数据字段（`af_uuid`、`af_os` 等，可能为正常 Apifox 数据）触发黄色提示；验证 Apifox 版本；在 /etc/hosts 中屏蔽所有 C2 域名 |
